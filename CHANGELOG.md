...
- `npx ai-first-os` (spec 067): a second way to install the system, alongside the git-clone prompt,
  which stays exactly as it was. Running it checks git and python3, copies the system to a fixed
  folder — `~/.ai-first-os`, replacing whatever was there — and prints the exact path of the skill
  that conducts the interview. It never installs system software, never asks a question, and never
  runs anything from what it just copied.
- `scripts/npm-package.sh`, which builds the npm package tree from the same source as
  `scripts/release-public.sh`: `core/`, `README.md`, `LICENSE`, plus `package.json` and `bin/`.

### Changed

- With Node present, the pack offered at the close of the bootstrap is one question, not two: there
  is nothing to defer when the download can run right now, and declining writes no task to the
  backlog. Without Node, the only exit stays "later" — the condition is the state of the machine,
  never how the system got installed.
